 ...
 *
 * يظهر عندما يفتح المستخدم قسم الإنجاز دون جلسة إنجاز بينما لديه جلسة صحيحة
 * في قسم الدردشة. "نعم" تزامن حساب الدردشة الحالي إلى الإنجاز بنفس البريد
 * (دون طلب تسجيل دخول Google مرة ثانية)، و"ليس الآن" تُبقي الحالة دون أي كسر.
 */
const SsoSyncDialog = () => {
  const { siblingSync, syncWithChat, dismissSiblingSync } = useAuth();
  ...
  const initials = (name || account?.email || "؟").slice(0, 2).toUpperCase();

  const handleSync = async () => {
    if (!account) return;
    setBusy(true);
    setError(null);
    const res = await syncWithChat();
    setBusy(false);
    if (!res.ok) {
      ...
      <DialogContent className="max-w-md">
        <DialogHeader className="text-center sm:text-center">
          ...
          <DialogDescription className="text-sm leading-relaxed">
            ...
            <span className="font-semibold text-foreground">الدردشة</span>.
            <br />
            هل تريد استخدام نفس الحساب والصورة في قسم{" "}
            <span className="font-semibold text-foreground">الإنجاز</span>؟
          </DialogDescription>
        </DialogHeader>
