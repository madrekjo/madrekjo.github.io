...
const Profile = () => {
  ...
  useEffect(() => {
    if (profile) {
      setDisplayName((profile as any).username || "");
    }
  }, [profile]);
  ...
  const nameUnchanged = displayName.trim() === ((profile as any)?.username ?? "").trim();

  const handleSaveName = async () => {
    try {
      await updateProfile.mutateAsync({ username: displayName.trim() });
