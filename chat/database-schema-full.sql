...
-- ============================================================================
-- ★★ نسخة REBUILD المعدّلة — يجب أن ترى فيها: realtime policy create skip ★★
-- ★★ ولاحقاً: cleanup rounds skip ★★
-- ============================================================================

-- 0) تنظيف ذاتي: إزالة كل سياسات التخزين القديمة (لو وُجدت) — حتى يتكرر البناء بسلام
DO $$
DECLARE r record;
BEGIN
  FOR r IN
...
EXCEPTION WHEN others THEN
  RAISE NOTICE 'realtime skip notifications: %', SQLERRM;
END $$;
...
EXCEPTION WHEN others THEN
  RAISE NOTICE 'realtime skip support_messages: %', SQLERRM;
END $$;
...
EXCEPTION WHEN others THEN
  RAISE NOTICE 'realtime skip round_chat: %', SQLERRM;
END $$;
...
EXCEPTION WHEN others THEN
  RAISE NOTICE 'realtime skip changes_messages: %', SQLERRM;
END $$;
...
-- 7) Realtime: lock down channel subscriptions (اختياري — يتجاوز عند عدم الصلاحية)
DO $$
DECLARE pol text;
BEGIN
  BEGIN
...
DO $$ BEGIN
  PERFORM public.delete_old_posts();
EXCEPTION WHEN others THEN RAISE NOTICE 'cleanup posts skip: %', SQLERRM; END $$;
...
DO $$ BEGIN
  PERFORM public.delete_old_rounds();
EXCEPTION WHEN others THEN RAISE NOTICE 'cleanup rounds skip: %', SQLERRM; END $$;
...
-- ============================================================================
-- 99) ضمان البث المباشر (Realtime) لجداول الدردشة الأساسية
-- (يتجاوز بأمان أي جدول لا يُسمح بإضافته في هذه البيئة)
-- ============================================================================
DO $$
DECLARE
  t text;
...
-- ============================================================================
-- 100) منح الصلاحيات (GRANTS) للجدوال بعد إعادة البناء — ضروري جداً
--      بدونها يظهر "تعذر تحميل الدردشة" / "حسابي مش محمل"
--      (قابل للتكرار بأمان)
-- ============================================================================
