...
const UserActionsDialog = ({ userId, open, onOpenChange, onChanged }: Props) => {
  ...
  useEffect(() => {
    ...
    setLoading(true);
    (async () => {
      const { data } = await supabase.from("profiles").select("user_id, full_name, avatar_url, is_banned, chat_banned, timeout_until, generation, gender, created_at, via_invite").eq("user_id", userId).maybeSingle();
      setProfile(data);
      setLoading(false);
    })();
  }, [userId, open]);
