...
import { Trash2, Check, X, ExternalLink, Loader2 } from "lucide-react";
...
const AdminReportsPanel = () => {
  ...
  const canManage = hasPermission("can_manage_reports");
  const canDeletePosts = hasPermission("can_delete_posts");
  ...
  const fetchReports = async () => {
    ...
    setReports(rows.map(r => ({ ...r, post: postMap.get(r.post_id), reporter: profMap.get(r.reporter_id) })));
    setLoading(false);
  };
  ...
  const setStatus = async (reportId: string, status: "resolved" | "dismissed") => {
    ...
    if (error) toast.error("فشل التحديث"); else { toast.success(status === "dismissed" ? "تم التجاهل" : "تم الحسم"); fetchReports(); }
  };

  if (!canManage && !isAdmin) {
    return <p className="text-sm text-muted-foreground text-center py-8">لا تملك صلاحية إدارة البلاغات</p>;
  }
              ...
              <CardContent className="py-4 space-y-3">
                ...
                {tab === "pending" && (
                  <div className="flex gap-2 flex-wrap justify-end">
                    ...
                    {r.post && !r.post.deleted_at && canDeletePosts && (
