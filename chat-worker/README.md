...
| `GET /feed?page=&limit=` | فيد رفيع: posts+commentCounts+likes+profiles (بلا أجسام تعليقات) | ساعة | بدل 5 طلبات من كل جهاز |
| `GET /comments?post_id=` | تعليقات + إعجاباتها + ملفات منشور واحد (لازي) | 5 دقائق | كاش مشترك لكل منشور |
...
التعليقات و `comment_likes` مخزّنة في مجموعة مستقلة (`comments`) منفصلة عن الفيد:
تعليق/لايك تعليق جديد يرفع stamp `comments` فيُحدَّث كاش تعليقات المنشورات فقط
ولا يكسّر كاش الفيد الرفيع لدى كل الأجهزة.
