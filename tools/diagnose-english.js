...
const ROOT = path.resolve('questions/2009/semester-1/english');
const KEYS = ['A', 'B', 'C', 'D'];
  ...
  for (const unit of units) {
    ...
    for (const file of files) {
      ...
      try { data = JSON.parse(fs.readFileSync(fp, 'utf8')); }
        ...
        report.push({ file: 'questions/2009/semester-1/english/' + unit + '/' + file, error: 'JSON تالف: ' + e.message });
        continue;
      }
      ...
      const entry = {
        file: 'questions/2009/semester-1/english/' + unit + '/' + file,
...
console.log('=== تقرير فحص الأسئلة (English) ===');
